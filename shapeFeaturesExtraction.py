...
################################### Main ###############################################################
'''
ori = cv.imread('C://Users//flavio//Documents//Dropbox//New database pictures2//light_dysplastic//204870926-204870933-002.BMP')
maskNucl = cv.imread('C://Users//flavio//Documents//Dropbox//New database pictures2//light_dysplastic_masc//masc_nuc//204870926-204870933-002-d.bmp')
maskCyto = cv.imread('C://Users//flavio//Documents//Dropbox//New database pictures2//light_dysplastic_masc//masc_junc//204870926-204870933-002-d.bmp')

'''

path = 'C://Users//flavio//Documents//Dropbox//New database pictures2//'
...
label = [0,0,0,1,1,1,0]
mask_nucl = 'masc_nuc'# {masc_nuc, masc_cyto, masc_junc}
mask_cyto = 'masc_junc'
...
cont = 0
for x in folders:
    arquivos_maskCyto = os.listdir(path+x+'_masc//'+mask_cyto)
    arquivos_maskNucl = os.listdir(path+x+'_masc//'+mask_nucl)
    contMask = 0
    print x
    for y in arquivos_maskCyto:
        if (y[-3:] == 'bmp') | (y[-3:] == 'BMP'):
            
            maskCyto = cv.imread(path+x+'_masc//'+mask_cyto+'//'+arquivos_maskCyto[contMask])
            maskCyto = cv.cvtColor(maskCyto,cv.COLOR_BGR2GRAY)
            
            maskNucl = cv.imread(path+x+'_masc//'+mask_nucl+'//'+arquivos_maskNucl[contMask])
            maskNucl = maskNucl[:,:,0]
...
np.savetxt("C:/Users/flavio/Desktop/shapeFeatures.csv",feature_vector, delimiter=",")

cv.imshow('image',maskNucl)
waitKey(0)